function writemidi_java(nm, filename, tpq, tempo,...
    tsig1, tsig2, patches, pitchwheel)
...
%      <time_frac_beats> <track> <chan> <pitch_wheel_val>.
% 
% NOTES:
...
% Pitchwheel array needs to be ints, requantize
% .. pitch wheel values
pitchwheel(:,4) = round(8192 + 4096*pitchwheel(:,4));
% .. and times (assumed provided in fractional beats)
pitchwheel(:,1) = round(tpq*pitchwheel(:,1));
% Subtract one from channels and tracks, to match internal
pitchwheel(:,[2 3]) = pitchwheel(:,[2 3]) - 1;
